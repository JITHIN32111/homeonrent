...
const { getLatestApprovedProperties } = propertyAPI();
...
const NewestDeals = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await getLatestApprovedProperties();
    ...
    <div className="py-10">
      ...
      <div className="deals grid 2xl:grid-cols-5 gap-8 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 py-10">
        {property.map((image) => (
          <div
            key={image._id}
            className="deal h-[350px] bg-white drop-shadow-2xl rounded-xl"
          >
            <div
              className="relative  h-[60%]"
              onClick={() => viewDetails(image._id)}
            >
              <img
                src={image.img[0].location}
                alt=""
